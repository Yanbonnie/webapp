    ...
    if (mainInfo.is_cd == 0 && mainInfo.status == 0 && mainInfo.is_prize == 0) { //可挖
      ...
      setTimeout(() => {
        if (mainInfo.is_prize == 1) {
          ...
          let go_sign_index = wx.getStorageSync('go_sign_index');
          // 不存在，设置本地缓存
          if (!go_sign_index) {
            wx.setStorageSync('go_sign_index', 1);
          } else {
            go_sign_index = go_sign_index + 1;
            wx.setStorageSync('go_sign_index', go_sign_index);
            this.setData({
              go_sign_index
            })
          }
